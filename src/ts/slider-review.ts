...
let initTranslateXPos = translateXPosList.find(el => el.mode === mode)?.pos as number;
...
const imagesBoxEl = document.querySelector('.slider-review__img-box') as HTMLDivElement;
...
navItemList = Array(imagesLength).fill({}, 0, imagesLength).map((_el, index) => {
